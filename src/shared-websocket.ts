...
export default class SharedWebsocket {
    ...
    async setUp(): Promise<void> {
        ...
        this._isMasterAlive = false // cleanup
        if (isMasterAlive) {
            return
        }

        localStorage.setItem('WANT_TO_BE_MASTER', this.uuid)
        const iWillBeMaster = await new Promise(
            resolve => setTimeout(
                () => {
    ...
    set onclose(fn: Function) {this._onclose = fn}

    get onerror(): Function {return this._onerror}
    set onerror(fn: Function) {this._onerror = fn}
    ...
    set onmessage(fn: Function) {this._onmessage = fn}
    ...
    set onopen(fn: Function) {this._onopen = fn}
}
